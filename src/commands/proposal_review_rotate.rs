...
pub fn run(cfg: &Config) -> Result<()> {
    ...
    let state_file = format!("{}-proposal", cfg.rotation_state_file);
    ...
    while tried < count {
        ...
        if cfg.dry_run {
            info!("dry-run: would process proposal review for project {project_root}");
            return Ok(());
        }

        fs::write(&state_file, idx.to_string()).ok();

        match super::proposal_review::run_unlocked(&ctx) {
            Ok(true) => return Ok(()),
            Ok(false) => {
